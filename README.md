...
示例：用户 0x70997970C51812dc3A010C7d01b50e0d17dc79C8 为 「**NFT 合约** 0xa82fF9aFd8f496c3d6ac40E2a0F282E47488CFc9」的**白名单用户**，可领取 `tokenId` 为 1 的 **NFT**，价格为 100 个 **ERC-20 token** 。

### 1. NFT 项目方构建白名单（Merkle Tree）

构建一个**白名单**，每个**叶节点**包含**用户地址**、所能够领取 **NFT** 的 `tokenId`、**NFT** Price（以 **ERC-20 token** 计量）
...
```solidity
0x000000000000000000000000a82ff9afd8f496c3d6ac40e2a0f282e47488cfc93266a47d1439207d12d08668bff456a38314b7eb399bee9c2cc49e2ce97478c2
```
...
**白名单数据**包含 <u>NFT 合约地址</u>与<u>白名单的 Merkle Root</u>，这个数据数据对应唯一的 **NFT** 合约与“优惠活动”，由项目方发送给白名单用户，或者直接公开。
...
### 1. {permitPrePay}：完成 ERC-20 token 预授权
...
### 2. {claimNFT}：验证用户是否位于白名单内，验证通过发放 NFT
